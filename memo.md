...
```
一番sizeが大きいメンバdが構造体のalign4バイトとなる。
またc配置直後はoffsetが5バイトで、dの4バイトの倍数ではないので次に大きい倍数8バイト目までパディングを行う。
また全て配置後のoffsetは13となりalignの倍数でないため、次に大きい16までパディングを行う。よって全体sizeは16。
...
|略称|意味|代入元例|代入先例|
|---|---|---|---|
|x|32bitレジスタに代入する場合のシンタックス。切り出しサイズは代入元レジスタに依存。|al, ax, |eax|
|xd|64bitレジスタに代入する場合。|eax, al, ax|rax|

上記はIntel記法の場合。
AT&Tの場合はもう少しレジスタの細分化が多い。参考までに以下の通り。
...
movオペランド例： 
* `movsx eax,al`<br>
32→8bitの符号付き拡張。余剰な上位ビットが符号付きで0詰めされる。
* `movzx eax,ax`<br>
32bit→16bitへの符号なし拡張。余剰な上位ビットが符号なしで0詰めされる。
* `movsxd rax,eax`<br>
32bit→64bitへの符号付き拡張。32bit以下→64bitのみ例外。
